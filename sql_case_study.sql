...
-- Q3 Number of indigo flights per month
...
-- Q4.list of flights that depart between 10AM and 2PM from banglore to new delhi 
...
-- Q5 number of flights departing on weekends from bangalore

SELECT *, DAYNAME(date_of_journey) FROM flights WHERE source='banglore'AND DAYOFWEEK(date_of_journey) IN (7,1);



-- Q6 calc the arrival time for all flights by adding duration to departure time
...
-- Q7  Number of flights which travell on multiple days

SELECT * FROM flights WHERE DAY(departure)!= DAY(arrival);



-- Q8 Avg DUration of flights bw all city pairs
...
-- Q9 quarterwsie number of flights for eac airline
...
-- Q10 longest flight distance (between cities in terms of time)
...
-- Q11 avg time duration of flights that have 1 stop vs 1+ stops
...
SELECT * FROM flights WHERE Airline='Air India' AND Source='Delhi'
 ...
 -- Q14 pairs of cities having average time duration> 3 hours
...
-- Q15 weekday vs time grid showing freq of flights from Banglore to Delhi 
...
-- Q16 weekday vs time grid showing price of flights from Banglore to Delhi
